...
author: Anthropic
content_type: official API documentation
retrieval_method: "WebFetch tool (direct HTML fetch)"
---
...
This pattern reduces model round-trips from N (one per region) to 1, processes large result sets programmatically before returning to Claude, and saves tokens by only returning aggregated conclusions instead of raw data.
...
For example, calling 10 tools directly uses approximately 10x the tokens of calling them programmatically and returning a summary.
...
**Advantages:** Simple to implement with minimal re-architecting; full control over environment and instructions.
...
**Advantages:** Safe programmatic tool calling on your own infrastructure; full control over the execution environment.
...
*Archived from Anthropic developer documentation on 2026-02-20. Full verbatim content retrieved via WebFetch tool.*
